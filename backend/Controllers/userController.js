...
exports.loginUser=catchAsyncError(async(req,res,next)=>{
   console.log('you hit login',req.body);
    ...
    const user= await User.findOne({email:email}).select("name email password");// we have unselected the password in schema 
...
exports.resetPassword=catchAsyncError(async(req,res,next)=>{
  ...
  console.log('your got the resettoken',resetPasswordToken);
    ...
    if(!user){
        return next(  new ErrorHandler("Reset password token is invlaiid or has been expired",404));
    }
...
if(req.body.newPassword!==req.body.confirmPassword){
    return next(new ErrorHandler("comfirmPassword and password should be same"));
}
...
exports.getAllUsers=catchAsyncError(async(req,res,next)=>{
    ...
    console.log("you got all user",users);
...
exports.updateUserRole=catchAsyncError(async(req,res,next)=>{
    ...
    const newUserData= {
       ...
       console.log("update user role is called",req.params.id);
